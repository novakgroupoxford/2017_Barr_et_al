...
dims = size(Mov3{1}); % dims to define when cells cross border
...
FVs = extractFeatures(Mov3,[], [], L); % First movie must be PCNA channel

%% 4. Select training data: skip if previous training matrices exist
% Load FVmatsel and P and skip if already performed previously into
% function positions 5 and 6. Otherwise select examples of cell sgments
% describing one cell, two cells, mitotic cells, or mitotic exit. 
...
Track = trackCells(Cost,dims,2); % final number is number of optimisation sweeps through tracks to make.
